...
def print_results(results: Dict, args:argparse.Namespace=None):
    ...
    for index in results:
        ...
        per_prob_res.append(np.mean(np.mean(problem_results) > 0))
...
def check_correctness(generated_code, input_outputs, timeout, debug):
    """Check correctness of code generation with a global timeout.
    The global timeout is to catch some extreme/rare cases not handled by the timeouts
    inside `run_test`"""
    def _temp_run(generated_code, input_outputs, debug, result):
        result.append(test_util.run_test(test=generated_code, in_outs=input_outputs, debug=debug))

    manager = multiprocessing.Manager()
    result = manager.list()
    p = multiprocessing.Process(target=_temp_run, args=(generated_code, input_outputs, debug, result))
    p.start()
...
def eval_and_save_problems(args):
    ...
    for index, problem in tqdm(problems.items()):
        # print(problem['answer'])
        res = []
        if not os.path.exists(args.save):
            os.makedirs(args.save)
        
        if not problem['input_output']:
            print("No test case available")
            continue

        if args.debug:
            print(f"\nTesting solution {index}")
        curr_res = [-2]
        try:
            curr_res = check_correctness(generated_code=problem['answer'], 
                                            input_outputs=problem['input_output'], 
                                            timeout=TIMEOUT, 
                                            debug=args.debug)
            fixed = []
            for e in curr_res:
